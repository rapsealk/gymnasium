...
EPSILON = 1.0
EPS_RANGE = (1.0, 0.02)
EPS_DECAY = (EPS_RANGE[0] - EPS_RANGE[1]) / EXPLORATION_STEPS
NUM_EPISODES = 500
...
class Brain:
    ...
    def __init__(self, num_states, num_actions, lr=1e-4):
        ...
        self.model = DQN(1, num_actions).to(device)
        print(self.model)
...
class Agent:
    ...
    def __init__(self, environment, n_states, n_actions, epsilon=EPSILON):
...
def preprocess(frame):
    ...
    frame = torchvision.transforms.functional.to_grayscale(frame, num_output_channels=1)
    frame = torchvision.transforms.functional.resize(frame, size=(84, 84))
...
if __name__ == "__main__":
    env = gym.make(ENV)
    agent = Agent(env, n_states=(1, 84, 84, 4), n_actions=env.action_space.n)

    for episode in range(NUM_EPISODES):     # episodes
